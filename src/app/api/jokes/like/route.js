...
export async function POST(req) {
  try {
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ error: 'Invalid or expired token. Relogin.' }),
        {
          status: 401,
          headers: { 'Content-Type': 'application/json' },
        }
    ...
    // Parse the request body to get the jokeId
    const { jokeId } = await req.json();
    if (!jokeId) {
      console.error('Joke ID is missing in request.');
      return new Response(
        JSON.stringify({ error: 'Joke ID is required.' }),
        {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        }
    ...
    if (!joke) {
      ...
      return new Response(
        JSON.stringify({ error: 'Joke not found.' }),
        {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
        }
    ...
    return new Response(
      JSON.stringify({
        ...
        message: userAlreadyLiked
          ? 'Like removed from the joke.'
          : 'Like added to the joke.',
        joke,
      }),
      {
        status: 200,
